<?php
	session_start();
	include_once '../../datos/AppUtils.php';
	include_once '../../vistas/utils.php';
	CheckLoginAccess();
?>
<div id='frmPersona' class='listform'>
<div class='form_top'>
	<span class='h1'>Personas</span>
	<hr class='separator'/>
	<div>
	<label for='txtBuscar'>Buscar:</label>
	<input type='text' id='txtBuscar' name='txtBuscar' placeholder='Ingrese búsqueda' />
	<a href='#' class='btn' id='btnRefrescar' name='btnRefrescar'>
		<img class='icon' src='../recursos/img/refresh.png'>
	</a>
	<a href='#' class='btn b_azul' id='btnNuevo' name='btnNuevo'>Nuevo</a>
</div>
</div>
<hr class='separator'/>
<div class='listform_body bpad15'>
	<div id='datos' class='centered' style='max-width: 800px;'></div>
</div>
</div>
<script>
var frm_pers = '#frmPersona';
$(document).ready(function(e){
	pers_mostrarDatos();
	$(frm_pers).find('#txtBuscar').focus();
	$(frm_pers).find('#txtBuscar').keyup(function(e) {
		pers_mostrarDatos();
	});
	$(frm_pers).find('#btnNuevo').off('click').click(function(e) {
		performLoad('vistas/persona/personaReg.php?parent=vistas/persona/persona.php');
	});
	$(frm_pers).find('#btnRefrescar').off('click').click(function(e) {
		pers_mostrarDatos();
	});
});
function pers_mostrarDatos() {
	var buscar = encodeURIComponent($('#txtBuscar').val());
	$(frm_pers).find('#datos').load('vistas/persona/personaList.php?b='+buscar);
}
function volver() {
	performLoad('vistas/persona/persona.php');
}
</script>