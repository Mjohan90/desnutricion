...
<div class='form_title'>
	<span class='h2'>Editar atención</span>
</div>
<hr class='separator'/>
<table class='form_data'>
	...
	<tr><td><label for='txtAtencFechaAtenc'>Fecha atenc:</label></td>
		<td><input type='text' id='txtAtencFechaAtenc' name='txtAtencFechaAtenc' value='<?php if ($atenc_row) { echo formatDate($atenc_row['atenc_fecha_atenc']); } ?>'  placeholder='Ingrese fecha atenc'/></td>
	</tr>
	<tr><td><label for='txtAtencObservacion'>Observacion:</label></td>
		<td><input type='text' id='txtAtencObservacion' name='txtAtencObservacion' value='<?php if ($atenc_row) { echo htmlspecialchars($atenc_row['atenc_observacion']); } ?>' maxlength='400' placeholder='Ingrese observacion'/></td>
	</tr>
	<tr><td><label for='txtAtencTratamiento'>Tratamiento:</label></td>
		<td><input type='text' id='txtAtencTratamiento' name='txtAtencTratamiento' value='<?php if ($atenc_row) { echo htmlspecialchars($atenc_row['atenc_tratamiento']); } ?>' maxlength='400' placeholder='Ingrese tratamiento'/></td>
	</tr>
	<tr><td><label for='txtAtencDieta'>Dieta:</label></td>
		<td><input type='text' id='txtAtencDieta' name='txtAtencDieta' value='<?php if ($atenc_row) { echo htmlspecialchars($atenc_row['atenc_dieta']); } ?>' maxlength='300' placeholder='Ingrese dieta'/></td>
	</tr>
	<tr><td><label for='txtAtencSituacion'>Situacion:</label></td>
		<td><input type='text' id='txtAtencSituacion' name='txtAtencSituacion' value='<?php if ($atenc_row) { echo $atenc_row['atenc_situacion']; } ?>'  placeholder='Ingrese situacion'/></td>
	</tr>
	<tr hidden><td><label for='txtAtencEstado'>Estado:</label></td>
		<td><input type='text' id='txtAtencEstado' name='txtAtencEstado' value='<?php if ($atenc_row) { echo $atenc_row['atenc_estado']; } ?>'  placeholder='Ingrese estado'/></td>
	</tr>
</table>
<hr class='separator'/>
<div class='form_foot'>
