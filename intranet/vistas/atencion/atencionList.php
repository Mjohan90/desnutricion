...
<table id='tblatencion' class='datatable'>
	...
	<tr>
		...
		<td><?php echo $row['atenc_situacion']; ?></td>
		<td class='txt_center' hidden><?php echo $row['atenc_fecha_reg']; ?></td>
		<td hidden><?php echo $row['atenc_estado']; ?></td>
		<td class='txt_center'><a href='#' onclick="atenc_editar('<?php echo $row['atenc_id']; ?>');">Editar</a></td>
		<td class='txt_center'><a href='#' onclick="atenc_borrar('<?php echo $row['atenc_id']; ?>');">Borrar</a></td>
	</tr>
	<?php } ?>
</table>
<script>
