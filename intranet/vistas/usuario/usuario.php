<?php
	session_start();
	include_once '../../datos/AppUtils.php';
	include_once '../../vistas/utils.php';
	CheckLoginAccess();
?>
<div id='frmUsuario' class='listform'>
<div class='form_top'>
	<span class='h1'>Usuarios</span>
	<hr class='separator'/>
	<div>
	<label for='txtBuscar'>Buscar:</label>
	<input type='text' id='txtBuscar' name='txtBuscar' placeholder='Ingrese búsqueda' />
	<a href='#' class='btn' id='btnRefrescar' name='btnRefrescar'>
		<img class='icon' src='../recursos/img/refresh.png'>
	</a>
	<a href='#' class='btn b_azul' id='btnNuevo' name='btnNuevo'>Nuevo</a>
</div>
</div>
<hr class='separator'/>
<div class='listform_body bpad15'>
	<div id='datos' class='centered' style='max-width: 800px;'></div>
</div>
</div>
<script>
var frm_usu = '#frmUsuario';
$(document).ready(function(e){
	usu_mostrarDatos();
	$(frm_usu).find('#txtBuscar').focus();
	$(frm_usu).find('#txtBuscar').keyup(function(e) {
		usu_mostrarDatos();
	});
	$(frm_usu).find('#btnNuevo').off('click').click(function(e) {
		performLoad('vistas/usuario/usuarioReg.php?parent=vistas/usuario/usuario.php');
	});
	$(frm_usu).find('#btnRefrescar').off('click').click(function(e) {
		usu_mostrarDatos();
	});
});
function usu_mostrarDatos() {
	var buscar = encodeURIComponent($('#txtBuscar').val());
	$(frm_usu).find('#datos').load('vistas/usuario/usuarioList.php?b='+buscar);
}
function volver() {
	performLoad('vistas/usuario/usuario.php');
}
</script>